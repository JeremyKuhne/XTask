...
internal ref struct BufferScope<T>
{
    ...
    /// <summary>
    ///  Increases the capacity by at least the specified amount.
    /// </summary>
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="increaseCapacityBy"/> was zero or negative.</exception>
    /// <exception cref="InvalidOperationException">The buffer was already at max capacity.</exception>
    public void IncreaseCapacity(int increaseCapacityBy)
    {
        if (increaseCapacityBy <= 0)
            throw new ArgumentOutOfRangeException(nameof(increaseCapacityBy));

        if (Length == int.MaxValue)
            throw new InvalidOperationException("Buffer is already at maximum capacity.");

        int newCapacity = Length + increaseCapacityBy;
        EnsureCapacity(newCapacity > 0 ? newCapacity : int.MaxValue);
    }
    ...
    ///  </para>
    /// </remarks>
    /// <param name="copy"><see langword="true"/> to copy the existing elements when new space is allocated.</param>
